...
SET extra_args=-lSDL2 -lOpenGL32 -I %CD%\lib\SDL2\include -L %CD%\lib\SDL2 -I %CD%\lib\microui 


REM AddCSource
REM ==========
REM If your project use C Source Files, add here the list of files you want to add to your build.
REM  SET addCSourceFile=%CD%\lib\microui\microui.c %CD%\lib\otherlib\otherlib.c
...
> bin/ReleaseStrip/obj/zig_report.txt (
  zig build-exe -O ReleaseSmall %rcmd% %libc% -fstrip -fsingle-threaded --color on -femit-bin=bin/ReleaseStrip/%ProjectName%.exe -femit-asm=bin/ReleaseStrip/obj/%ProjectName%.s -femit-llvm-ir=bin/ReleaseStrip/obj/%ProjectName%.ll -femit-llvm-bc=bin/ReleaseStrip/obj/%ProjectName%.bc -femit-h=bin/ReleaseStrip/obj/%ProjectName%.h -ftime-report -fstack-report %extra_args% --name %ProjectName% main.zig %addCSourceFile% 
) 2>&1 
