...
//   Mike Shah - https://www.youtube.com/watch?v=RRWgfMfrOs4
...
const cameraType = struct {
  ...
  pub fn GetViewMatrix(self: *Self) glm.mat4Type { 
    return glm.lookAt(self.eye, self.viewDirection, self.upVector); 
  }
  ...
  fn move(self: *Self, dir: u8, value: f32) void { self.eye[dir] += value; }
...
fn InitializeProgram() void {
  ...
  gAppWindow = sdl.SDL_CreateWindow("SDL3 OpenGL: [Arrow,Pg Down/Up]=Position, [1..6]=Rotation, [IJKLUO]=Scale | SHIFT+ESC = Quit", gScreenWidth, gScreenHeight, sdl.SDL_WINDOW_OPENGL);
  if (gAppWindow == null) {
...
fn PreDraw() void {
  ...
  // Camera View
  var ViewMatrix: glm.mat4Type = gCamera.GetViewMatrix();
  ...
  sdl.glUniformMatrix4fv(gUniformLocation_uViewMatrix, 1, sdl.GL_FALSE, &ViewMatrix[0][0]);
}
...
fn CreateGraphicsPipeline() void {
  ...
  // Prepare to transfer Uniform Variables from CPU to GPU GraphicsPipeline
  gUniformLocation_uOffset = sdl.glGetUniformLocation(gGraphicsPipelineShader, "u_Offset");
  if (gUniformLocation_uOffset < 0)
    print("Could not find u_Offset location, maybe a mispelling?\n", .{ });

  gUniformLocation_uModelMatrix = sdl.glGetUniformLocation(gGraphicsPipelineShader, "u_ModelMatrix");
  if (gUniformLocation_uModelMatrix < 0) 
    print("Could not find u_ModelMatrix location, maybe a mispelling?\n", .{ });

  gUniformLocation_uViewMatrix = sdl.glGetUniformLocation(gGraphicsPipelineShader, "u_ViewMatrix");
  if (gUniformLocation_uViewMatrix < 0) 
    print("Could not find u_ViewMatrix location, maybe a mispelling?\n", .{ });
  ...
  const zFar: f32 = 10.0; // Far clipping plane
  g_uProjection = glm.perspective(fov, aspect, zNear, zFar);

  gUniformLocation_uProjection = sdl.glGetUniformLocation(gGraphicsPipelineShader, "u_Projection");
  if (gUniformLocation_uProjection < 0) 
    print("Could not find u_Projection location, maybe a mispelling?\n", .{ });
