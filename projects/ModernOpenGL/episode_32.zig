...
//   Mike Shah - https://www.youtube.com/watch?v=Q8aQ1t70k-s
...
const cameraType = struct {
  ...
  pub fn GetViewMatrix(self: *Self) glm.mat4Type { 
    return glm.lookAt(self.eye, self.eye + self.viewDirection, self.upVector); 
  }
  pub fn MoveLeft(self: *Self) void     { self.move(0, -self.speed); }
  pub fn MoveRight(self: *Self) void    { self.move(0,  self.speed); }
  pub fn MoveUp(self: *Self) void       { self.move(1, -self.speed); }
  pub fn MoveDown(self: *Self) void     { self.move(1,  self.speed); }
  pub fn MoveForward(self: *Self) void  { self.move(2, -self.speed); }
  pub fn MoveBackward(self: *Self) void { self.move(2,  self.speed); }
  fn move(self: *Self, dir: u8, value: f32) void { 
    self.eye[dir] += value; 
    //self.eye[dir] += value; 
  }
  ...
  pub fn MouseLook(self: *Self, mouseX: f32, mouseY: f32) void { 
    ...
    self.viewDirection = glm.rotateVector(
      self.viewDirection, 
      -mouseY, 
      glm.vec3{ 1.0, 0.0, 0.0 }); // RightVector;
            
    self.mousePosition.x += mouseX; self.mousePosition.y += mouseY;
    //print("x:{d:>8.5}\ty:{d:>8.5}\tmpX:{d:>8.5}\tmpY:{d:>8.5}\n", .{ mouseX, mouseY, self.mousePosition.x, self.mousePosition.y});
    //print("viewDirection: {}\n", .{self.viewDirection});
  }
...
fn CreateGraphicsPipeline() void {
  ...
  std.debug.print("{}\t{}\t{}\t{}\n", .{gUniformLocation_uOffset, gUniformLocation_uModelMatrix, gUniformLocation_uViewMatrix, gUniformLocation_uProjection});
