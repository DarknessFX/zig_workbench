...
//   Mike Shah - https://www.youtube.com/watch?v=rSrlwQO28sk
...
\\
\\void main() {
\\  color = vec4(v_colors.r + u_Offset.r, v_colors.g + u_Offset.g, v_colors.b + u_Offset.b, 1.0f);
\\}
;
...
fn InitializeProgram() void {
  ...
  gAppWindow = sdl.SDL_CreateWindow("SDL3 OpenGL: [Arrow,Pg Down/Up]=Position, [1..6]=Rotation, [WASDQE]=Scale", gScreenWidth, gScreenHeight, sdl.SDL_WINDOW_OPENGL);
  if (gAppWindow == null) {
...
fn Input() void {
  ...
  if (keyboardstate[sdl.SDL_SCANCODE_A]         != 0)  { g_uScale[0] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_D]         != 0)  { g_uScale[0] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_W]         != 0)  { g_uScale[1] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_S]         != 0)  { g_uScale[1] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_Q]         != 0)  { g_uScale[2] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_E]         != 0)  { g_uScale[2] -= 0.01; }
...
fn PreDraw() void {
  ...
  g_uModelMatrix = glm.rotate(g_uModelMatrix, g_uRotate[0], glm.vec3{ 1.0, 0.0, 0.0});
  g_uModelMatrix = glm.rotate(g_uModelMatrix, g_uRotate[1], glm.vec3{ 0.0, 1.0, 0.0});
  g_uModelMatrix = glm.rotate(g_uModelMatrix, g_uRotate[2], glm.vec3{ 0.0, 0.0, 1.0});
  ...
  // Transfer UniformLocation from CPU to GPU (Shaders)
