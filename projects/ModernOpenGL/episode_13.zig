...
//   Mike Shah - https://www.youtube.com/watch?v=eYMAR5bW8eo
...
var gVertexBufferObject_Positions : sdl.GLuint = 0; // VBO
var gVertexBufferObject_Colors : sdl.GLuint = 0; // VBO
...
fn Draw() void {
  ...
  sdl.glBindBuffer(sdl.GL_ARRAY_BUFFER, gVertexBufferObject_Positions);
  sdl.glBindBuffer(sdl.GL_ARRAY_BUFFER, gVertexBufferObject_Colors);
  // Render data
...
fn VertexSpecification() void {
  ...
  const vertexVectorLen = 9;
  const vertexPositions = @Vector(vertexVectorLen, sdl.GLfloat){
  //  x     y    z
    -0.8, -0.8, 0.0, // vertex 1, left
     0.8, -0.8, 0.0, // vertex 2, right
     0.0,  0.8, 0.0  // vertex 3, top
  };

  const vertexColors = @Vector(vertexVectorLen, sdl.GLfloat){
  //  r     g    b
     1.0, 0.0, 0.0, // vertex 1, left Red
     0.0, 1.0, 0.0, // vertex 2, right Green
     0.0, 0.0, 1.0  // vertex 3, top Blue
  };
  ...
  // content.
  sdl.glGenBuffers(1, &gVertexBufferObject_Positions);
  sdl.glBindBuffer(sdl.GL_ARRAY_BUFFER, gVertexBufferObject_Positions);
  ...
  sdl.glBufferData(
    ...
    &vertexPositions, // Raw array of data
  ...
  sdl.glVertexAttribPointer(  // Setup the attribute
    ...
    0, // Stride (when you want to declare an space betweeen components)
    null); // Offset

  // Setting up our VBO colors
  sdl.glGenBuffers(1, &gVertexBufferObject_Colors);
  sdl.glBindBuffer(sdl.GL_ARRAY_BUFFER, gVertexBufferObject_Colors);
  sdl.glBufferData(
    sdl.GL_ARRAY_BUFFER,  // Kind of buffer we are working with
    vertexVectorLen * @sizeOf(sdl.GLfloat), // Size of data in bytes
    &vertexColors, // Raw array of data
    sdl.GL_STATIC_DRAW); //How we intend to use the data

  // Now linking up the attributes in our VAO
  sdl.glEnableVertexAttribArray(1);  // Creates Attribute at position (or layout) 0.
  sdl.glVertexAttribPointer(  // Setup the attribute
