...
//   Mike Shah - https://www.youtube.com/watch?v=6uIzWeCk-do
...
\\
\\void main() {
\\  color = vec4(v_colors.r, v_colors.g + u_Offset, v_colors.b, 1.0f);
\\}
;
...
fn Input() void {
  ...
  if (keyboardstate[sdl.SDL_SCANCODE_UP]   != 0)  { g_uOffset += 0.01; } //print("g_uOffset: {d:0.5}\n", .{ g_uOffset }); }
  if (keyboardstate[sdl.SDL_SCANCODE_DOWN] != 0)  { g_uOffset -= 0.01; } //print("g_uOffset: {d:0.5}\n", .{ g_uOffset }); }
}
...
fn PreDraw() void {
  ...
  // Prepare to transfer Uniform Variable from CPU to GPU
  const uniformlocation: sdl.GLint = sdl.glGetUniformLocation(gGraphicsPipelineShader, "u_Offset");
  // Check if glGetUniformLocation was successful
  if (uniformlocation >= 0) {
    // Transfer g_uOffset (CPU) to u_Offset (GPU, Shaders)
    sdl.glUniform1f(uniformlocation, g_uOffset);

    //print("u_Offset Location: {d:0.5}\n", .{ uniformlocation });
  } else {
