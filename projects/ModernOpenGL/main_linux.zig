...
//
// Linux version:
//    Changed TASKS.JSON from linking with "-lopengl32" (windows) to "-lGL" (linux)
//    Tested with VMWare.
//    Arch Linux x86_64 6.9.7, Gnome Wayland.
//    Built SDL3 version 3.1.1 .
//
//  Default LOG: 
...
//    Version : 4.3 (Core Profile) Mesa 24.1.2-arch1.1
//    Shader  : 4.30
...
// Screen dimensions
const gScreenWidth : c_int = 1280;
const gScreenHeight : c_int = 720;
...
// Uniform Variables, CPU to GPU, shared with shaders.
// Locations (from glUniformLocations) in OpenGL buffer where to copy data to GPU.
var gUniformLocation_uOffset: sdl.GLint = -1;
var gUniformLocation_uModelMatrix: sdl.GLint = -1;
var gUniformLocation_uProjection: sdl.GLint = -1;
var gUniformLocation_uViewMatrix: sdl.GLint = -1;

// Values used to perform Matrix Transformation to our object.
var g_uOffset: [3]f32 = .{ 0.0, 0.0, -2.0};
var g_uRotate: [3]f32 = .{ 0.0, 0.0, 0.0};
var g_uScale:  [3]f32 = .{ 1.0, 1.0, 1.0};
var g_uModelMatrix: glm.mat4Type = glm.mat4;
var g_uProjection: glm.mat4Type = glm.mat4;

// Our Camera object
const cameraType = struct {
  eye: glm.vec3,
  viewDirection: glm.vec3,
  speed: f32,

  // Assume we start on a perfect plane
  pub const upVector: glm.vec3 = .{ 0.0, 1.0, 0.0 };
  pub const rightVector: glm.vec3 = .{ 1.0, 0.0, 0.0 };
  pub const forwardVector: glm.vec3 = .{ 0.0, 0.0, -1.0 };
  ...
  pub fn Init() Self {
    return Self{
      ...
      // NOTE: This is along '-z' because otherwise, we'd be looking
      //       begind us.
  ...
  pub fn GetViewMatrix(self: *Self) glm.mat4Type { 
    return glm.lookAt(self.eye, self.eye + self.viewDirection, upVector); 
  }

  pub fn MoveRight(self: *Self) void    { self.move(upVector,       self.speed); }
  pub fn MoveLeft(self: *Self) void     { self.move(upVector,      -self.speed); }
  pub fn MoveForward(self: *Self) void  { self.move2(self.speed); }
  pub fn MoveBackward(self: *Self) void { self.move2(-self.speed); }
  pub fn MoveUp(self: *Self) void       { self.move(rightVector,    self.speed); }
  pub fn MoveDown(self: *Self) void     { self.move(rightVector,   -self.speed); }

  fn move(self: *Self, dir: glm.vec3, value: f32) void { 
    self.eye += glm.mulFloat(glm.cross(self.viewDirection, dir), value); }
  fn move2(self: *Self, value: f32) void { 
    self.eye += glm.mulFloat(glm.normalize(self.viewDirection - self.eye), value); }

  pub fn MouseLook(self: *Self, mouseX: f32, mouseY: f32) void {
    const curUpVector: glm.vec3 = glm.cross(upVector, self.viewDirection);
    const curRightVector: glm.vec3 = glm.cross(curUpVector, self.viewDirection);

    self.viewDirection = glm.rotateVector(
      self.viewDirection, 
      mouseX * self.speed, // Reduce the mouse speed for smooth view
      curRightVector);

    self.viewDirection = glm.rotateVector(
      self.viewDirection, 
      mouseY * self.speed,  // Reduce the mouse speed for smooth view
      curUpVector);
  }
};
// Create and initialize a single global camera
var gCamera: cameraType = cameraType.Init();
...
\\
\\void main() {
\\  color = vec4(v_colors.r + u_Offset.r, v_colors.g + u_Offset.g, v_colors.b + (u_Offset.b + 2.0), 1.0f);
\\}
;
...
fn MainLoop() void {
  ...
  while(!gQuit) {
    ...
    // place before draw calls. PreDraw() renamed to Update()
    Update();
...
fn InitializeProgram() void {
  ...
  // Setup Mouse capture
  sdl.SDL_WarpMouseInWindow(gAppWindow, gScreenWidth / 2, gScreenHeight / 2);
...
fn Input() void {
  ...
  while(sdl.SDL_PollEvent(&evt) != 0) {
    ...
    if (evt.type == sdl.SDL_EVENT_MOUSE_MOTION) {
      gCamera.MouseLook(evt.motion.xrel, evt.motion.yrel);
    }
  ...
  // Change Matrix Transformation Values
  // ArrowKeys + PageDown/PageUp = Change Position, g_uOffset
  // Number 1 to 6 = Change Rotation, g_uRotate
  // WASDQE = Change Scale, g_uScale
  const keyboardstate: [*c]const u8 = sdl.SDL_GetKeyboardState(null);
  if (keyboardstate[sdl.SDL_SCANCODE_RIGHT]     != 0)  { g_uOffset[0] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_LEFT]      != 0)  { g_uOffset[0] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_UP]        != 0)  { g_uOffset[1] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_DOWN]      != 0)  { g_uOffset[1] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_PAGEUP]    != 0)  { g_uOffset[2] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_PAGEDOWN]  != 0)  { g_uOffset[2] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_1]         != 0)  { g_uRotate[0] += 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_2]         != 0)  { g_uRotate[0] -= 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_3]         != 0)  { g_uRotate[1] += 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_4]         != 0)  { g_uRotate[1] -= 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_5]         != 0)  { g_uRotate[2] += 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_6]         != 0)  { g_uRotate[2] -= 1.0; }
  if (keyboardstate[sdl.SDL_SCANCODE_J]         != 0)  { g_uScale[0] += 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_L]         != 0)  { g_uScale[0] -= 0.01; }
  if (keyboardstate[sdl.SDL_SCANCODE_I]         != 0)  { g_uScale[1] += 0.01; }
  ...
  if (keyboardstate[sdl.SDL_SCANCODE_S]         != 0)  { gCamera.MoveBackward(); }
  if (keyboardstate[sdl.SDL_SCANCODE_Q]         != 0)  { gCamera.MoveUp(); }
  if (keyboardstate[sdl.SDL_SCANCODE_E]         != 0)  { gCamera.MoveDown(); }

  // Keyboard: LSHIFT+ESC = Quit
  if (keyboardstate[sdl.SDL_SCANCODE_ESCAPE] != 0 
  and keyboardstate[sdl.SDL_SCANCODE_LSHIFT] != 0) { gQuit = true; }

  //print("g_uOffset: {d:>8.5}\n", .{ g_uOffset });
  //print("g_uRotate: {d:>8.5}\n", .{ g_uRotate });
  //print("g_uScale : {d:>8.5}\n", .{ g_uScale });
}
...
fn Update() void {
  ...
  // Reset ModelMatrix to base IdentityMatrix values.
  g_uModelMatrix = glm.mat4;

  // Update our model matrix applying Translate
  g_uModelMatrix = glm.translate(g_uModelMatrix, g_uOffset);

  // Update our model matrix applying Rotation
  g_uModelMatrix = glm.rotateAll(g_uModelMatrix, g_uRotate);

  // Update our model matrix applying Scale
  g_uModelMatrix = glm.scale(g_uModelMatrix, g_uScale);

  // Transfer UniformLocation "g_uVariables" from CPU to GPU (Shaders)
  sdl.glUniform3f(gUniformLocation_uOffset, g_uOffset[0], g_uOffset[1], g_uOffset[2] );
  sdl.glUniformMatrix4fv(gUniformLocation_uModelMatrix, 1, sdl.GL_FALSE, &g_uModelMatrix[0][0]);
  sdl.glUniformMatrix4fv(gUniformLocation_uProjection, 1, sdl.GL_FALSE, &g_uProjection[0][0]);
  sdl.glUniformMatrix4fv(gUniformLocation_uViewMatrix, 1, sdl.GL_FALSE, &gCamera.GetViewMatrix()[0][0]);
}
...
fn Draw() void {
  ...
  // Close our current graphics pipeline.
...
fn CreateGraphicsPipeline() void {
  ...
  // NOTE: This code is at different function than episodes. He mention that it was wasteful
  // to GetUniformLocation every frame in PreDraw() and I agree, this locations are needed 
  // only once, so I get them at CreateGraphicsPipeline() and save to global variables.

  // Get OpenGL UniformVariable locations to transfer from CPU to GPU GraphicsPipeline
  GetUniformLocation(&gUniformLocation_uOffset,       "u_Offset");
  GetUniformLocation(&gUniformLocation_uModelMatrix,  "u_ModelMatrix");
  GetUniformLocation(&gUniformLocation_uViewMatrix,   "u_ViewMatrix");
  GetUniformLocation(&gUniformLocation_uProjection,   "u_Projection");

  // Projection Matrix (Perspective)
  const fov: f32 = glm.radians(45.0); // Field of view angle in radians
  const aspect: f32 = @as(f32, @floatFromInt(gScreenWidth)) / @as(f32, @floatFromInt(gScreenHeight)); // Aspect ratio
  const zNear: f32 = 0.1; // Near clipping plane
  const zFar: f32 = 100.0; // Far clipping plane
  g_uProjection = glm.perspective(fov, aspect, zNear, zFar);
}
...
/// Helper function to avoid repeating glGetUniformLocation for each variable.
/// @param *location: the gUniformLocation_Variable where we store the OpenGL 
///    buffer to copy data. Pass as &reference since we are changing the value 
///    directly.
/// @param name: String with the name of Uniform Variable, must be exactly the 
///    same name used in the Shaders.
inline fn GetUniformLocation(location: *sdl.GLint, comptime name: [:0]const u8) void {
  location.* = sdl.glGetUniformLocation(gGraphicsPipelineShader, name);
  // Check if we got the Location successfully
  if (location.* < 0)
    print("Could not find " ++ name ++ " location, maybe a mispelling?\n", .{ });
}
