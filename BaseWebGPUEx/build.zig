...
const projectname: []const u8 = "BaseWebGPUEx";
...
pub fn buildWasm(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) !void {
  ...
  wasm.addIncludePath( b.path("lib/webgpu") );
  wasm.addIncludePath( b.path("lib/SDL2/include") );
  wasm.addIncludePath( b.path("lib/imgui") );
  wasm.addIncludePath( b.path("lib/imgui/backends") );    
  ...
  wasm_cmd.addArgs(&[_][]const u8{
    "-o",
    b.fmt("{s}/{s}_Shared.html", .{ path, projectname }),
    "-Oz",
...
pub fn buildWebGPU(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) !void {
  ...
  lib.addIncludePath( b.path("lib/webgpu"));

  lib.addIncludePath( b.path("lib/imgui") );
  lib.addIncludePath( b.path("lib/imgui/backends") );

  lib.addIncludePath( b.path("lib/SDL2/include"));
  lib.addLibraryPath( b.path("lib/SDL2") );
  ...
  const imgui_srcs = .{
    ...
    "lib/imgui/backends/imgui_impl_wgpu_emscripten.cpp",
  };
  ...
  emcc_cmd.addArgs(&[_][]const u8{ 
    ...
    fmt("-I{s}\\{s}", .{ current_path, "lib\\wasm_webgpu" }),
    fmt("-I{s}\\{s}", .{ current_path, "lib\\imgui_web" }),
    fmt("-I{s}\\{s}", .{ current_path, "lib\\SDL2\\include" }),
    "-sWASM=1",
    "-sSINGLE_FILE=1",
    "-sUSE_WEBGPU=1",
    "-sUSE_SDL=2",
