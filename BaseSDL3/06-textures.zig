...
pub export fn SDL_AppInit(appstate: ?*anyopaque, argc: c_int, argv: [*][*]u8) sdl.SDL_AppResult {
  ...
  const appTitle = "SDL3 Example Renderer Textures";
  _ = sdl.SDL_SetAppMetadata(appTitle, "1.0", "com.example.renderer-textures");
...
pub export fn SDL_AppIterate(appstate: ?*anyopaque) sdl.SDL_AppResult {
  ...
  const ftexture_height: f32 = @floatFromInt(texture_height);
  {
    ...
    dst_rect.y = 0.0;
    dst_rect.w = ftexture_width;
    dst_rect.h = ftexture_height;
    _ = sdl.SDL_RenderTexture(renderer, texture, null, &dst_rect);
  }

  {
    dst_rect.x = @as(f32, @floatFromInt(WINDOW_WIDTH - texture_width)) / 2.0;
    dst_rect.y = @as(f32, @floatFromInt(WINDOW_HEIGHT - texture_height)) / 2.0;
    dst_rect.w = ftexture_width;
    dst_rect.h = ftexture_height;
    _ = sdl.SDL_RenderTexture(renderer, texture, null, &dst_rect);
  }
  ...
  {
    dst_rect.x = @as(f32, @floatFromInt(WINDOW_WIDTH - texture_width)) - (100.0 * scale);
    dst_rect.y = @as(f32, @floatFromInt(WINDOW_HEIGHT - texture_height));
    dst_rect.w = ftexture_width;
    dst_rect.h = ftexture_height;
    _ = sdl.SDL_RenderTexture(renderer, texture, null, &dst_rect);
  }
