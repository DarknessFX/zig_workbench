...
pub export fn wWinMain(hInstance: win.HINSTANCE, hPrevInstance: ?win.HINSTANCE, 
  ...
  vk_state.hInstance = vk.GetModuleHandleA(null);
  vk_state.window = vk.FindWindowA(wnd.classname, wnd.name);

  initVulkan() catch unreachable;
  MainLoop();
  deinit();
  win.Destroy();
  return 0;
...
fn createSurface() !void {
  const createInfo = vk.VkWin32SurfaceCreateInfoKHR{
    ...
    .pNext = null,
    .flags = 0,
    .hinstance = vk_state.hInstance,
    .hwnd = vk_state.window,
  };
  try vkcheck(vk.vkCreateWin32SurfaceKHR(vk_state.instance, &createInfo, null, &vk_state.surface), "Failed to Create Win32 Surface KHR");
  if (vkdebug_mode) std.debug.print("Created VK Win32 Surface KHR : {any}\n", .{vk_state.surface});
}
