...
//#region MARK: UTIL
//=============================================================================
fn WindowProc( hWnd: win.HWND, uMsg: win.UINT, wParam: usize, lParam: isize ) callconv(.winapi) isize {
  switch (uMsg) {
    WM_DESTROY,
    WM_CLOSE => {
      PostQuitMessage(0);
...
fn CreateWindow(hInstance: win.HINSTANCE, nCmdShow: win.INT) void {
  const wnd_class: wnd.WNDCLASSEXW = .{
  ...
  _ = AdjustWindowRectEx(&wnd_size, WS_OVERLAPPEDWINDOW, win.BOOL.FALSE, WS_EX_APPWINDOW);
  ...
  _ = SetWindowPos(hwnd, null, wnd_size.left, wnd_size.top, wnd_size.right, wnd_size.bottom, SWP_NOCOPYBITS );
